...
class DataLoader(object):
    ...
    def __init__(self, 
        ...
        self.flow_path = PATH["preproc"]+"optfl_i64_train/"
    ...
    def initialize(self, x_shared, y_shared, m_shared, start=True):
        ...
        self.loader_path_flow = "/dev/shm/loader/flow%f"%time()
        ...
        for i in range(self.n_mem_slots): 
            ...
            path = self.flow_path + str(video_number).zfill(4) + ".npy"
            shutil.copyfile(path, self.loader_path_flow + "%i_%i" % (i, video_number))
    ...
    def memory_loader(self):
        while not self.is_terminated.value:
            ...
            video_number = split.train_idxs[npr.randint(len(split.train_idxs))]
            path = self.data_path+str(video_number).zfill(4)+".npy"
            path_flow = self.flow_path+str(video_number).zfill(4)+".npy"
            ...
            os.remove(old_path)
            old_path = glob(self.loader_path_flow+"%i_*"%mid)[0]
            os.remove(old_path)
            shutil.copyfile(path, self.loader_path+"%i_%i"%(mid, video_number))
            shutil.copyfile(path_flow, self.loader_path_flow+"%i_%i"%(mid, video_number))
    ...
    def load_video(self):
        ...
        path = glob(self.loader_path_flow + "%i_*" % mid)[0]
        flow = np.load(path)

        video = np.concatenate((video, flow), axis=1)
    ...
    def create_valid_generator(self, n_samples=len(split.valid_idxs)):
        ...
        for b in range(n_batches):    
            ...
            for idx in range(self.batch_size):
                ...
                smpl = np.load(path)
                path = self.flow_path + str(split.valid_idxs[self.batch_size*b+idx]).zfill(4)+".npy"
                flow = np.load(path)

                smpl = np.concatenate((smpl, flow), axis=1)
