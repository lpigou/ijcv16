...
target_class = 3
class_name = "perfetto"
labels = load(PATH["labels"] + "labels_raw.npy")
found = False
for smp_idx, lbl in enumerate(labels):
    if smp_idx<10: continue
    for FRAME, l in enumerate(lbl):
        if l == target_class:
            found = True
            break
    if found: break
...
def main():
    ...
    extract_rgbd(file_paths[smp_idx])
...
def extract_rgbd(file_path, subset="train"):
    try:    
        ...
        for i in range(n_frames):
            ...
            if i >= FRAME and i <= FRAME+30:
                img = img[...,[2, 1, 0]]
                # img = cv2.resize(img, (512, 512), interpolation=cv2.INTER_NEAREST)
                print img.shape, n_frames
                scipy.misc.toimage(img, cmin=0.0, cmax=255.0).save('imgs/%s%s.png'%(class_name, i))
                # break
