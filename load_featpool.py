...
class DataLoader(object):
    ...
    def __init__(self, 
        ...
        self.data_path = data_path = "data/"+data_path+"/"
        self.labels_path = labels_path = "data/"+labels_path
        self.n_jobs = n_jobs
        ...
        self.y_shape = (chunk_size, n_classes)
    ...
    def initialize(self, x_shared, y_shared, m_shared, start=True):
        ...
        self.zeros_x = np.zeros((self.seq_length//2, self.n_channels)+self.im_shape,dtype="float32")
        # self.zeros_y = np.zeros((self.seq_length//2, self.n_classes),dtype="float32")
        # self.zeros_y[:,0] = 1.

        # for i, label in enumerate(labels):
        #     labels[i] = np.concatenate((self.zeros_y[:-1], label, self.zeros_y), axis=0)
    ...
    def slave(self, indices): 
        ...
        # video = np.concatenate((self.zeros_x[:-1], video, self.zeros_x), axis=0)

        # calculate wich frame indices will be used
        t_scale = npr.uniform(*self.aug_params["t_scale"])
        orig_size = int(np.round(self.seq_length * t_scale * self.step_size))
        ...
        for i in range(sample.shape[0]):
            for j in range(sample.shape[1]):
                ...
                img = fast_warp(img, at.params, output_shape=self.im_shape)
                aug_sample[i,j] = img

        self.x_shm[-1].get()[idx_chunk] = aug_sample
        label = self.labels_shm.get()[idx_train-1]
        self.y_shm[-1].get()[idx_chunk] = label[sample_idxs[self.seq_length//2-1]]
    ...
    def create_train_generator(self):
        ...
        if self.block:
            ...
            sleep(5)
            while not self.is_terminated.value:
                self.x_shared.set_value(x[-1], borrow=False)
                self.y_shared.set_value(y[-1], borrow=False)
                yield
    ...
    def create_valid_generator(self, n_samples=len(split.valid_idxs)):
        ...
        for b in range(n_batches):    
            ...
            valid_len = int(np.ceil(max_len / float(self.seq_length) )) * self.seq_length
            ...
            for idx in range(self.batch_size):
                ...
                # true_len = len(smpl)
                smpl = np.concatenate((self.zeros_x[:-1], smpl, self.zeros_x), axis=0)
...
def test_generator():
    loader = DataLoader(
        ...
        n_mem_slots=10,
        block=True)
        # aug_params={'t_scale':(1/2.,2),'scale':(1/1.2,1.2),'shear':(-3,3),'rot':(-3,3),'trans_x':(-15,15), 'trans_y':(-10,10)})
