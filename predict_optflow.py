...
if not (2 <= len(sys.argv) <= 4):
    sys.exit("Usage: %s <metadata_path> [subset=test] [batch_size=5]"%os.path.basename(__file__))
...
if not os.path.exists(target_path):
    ...
    # output_pred = model.out_reshape.get_output(deterministic=True, mask=mask_batch)
...
if subset == "test":
    ...
    if not os.path.exists(target_path):
        ...
        flow_path = cfg.data_loader.flow_path.replace("train", "test")
        ...
        def load_batch(b):
            ...
            for idx in range(cfg.batch_size):
                ...
                path = flow_path+str(split.test_idxs[cfg.batch_size*b+idx]).zfill(4)+".npy"
                flow = np.load(path)
    ...
    labels = np.load(PATH["labels"]+"labels_raw_test.npy")
    # labelsn = labels.copy()
