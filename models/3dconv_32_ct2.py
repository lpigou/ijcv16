...
grad_drop_updates = 500

# SGD
############
slow_start = dict(epochs=500, lr=3e-6)
learning_rate = 3e-4
...
# conv3d_layer = partial(layers.NervanaConvLayer, 
#     stride=None,
#     border_mode="same", 
#     W=nn.init.Orthogonal("relu"),
#     b=nn.init.Constant(0.1),
...
maxpool = conv3d.MaxPool2DLastLayer

conv3d_layer = partial(conv3d.Conv3DLayer, 
    W=nn.init.Orthogonal("relu"),
    b=nn.init.Constant(0.1), 
    nonlinearity=nn.nonlinearities.LeakyRectify(leakiness))
...
def maxpool(l, s, ds):
    # s = seq_length
    b = batch_size
    _,c,h,w = l.get_output_shape()
    l = nn.layers.ReshapeLayer(l, (b,s,c,h,w) )
    l = nn.layers.DimshuffleLayer(l, (0,2,1,3,4))
    l = conv3d.MaxPool3DLayer(l, ds=ds)
    return nn.layers.DimshuffleLayer(l, (0,2,1,3,4))

dense_layer = partial(nn.layers.DenseLayer,
    # W=nn.init.Orthogonal("relu"),
    W=nn.init.Normal(0.01),
    b=nn.init.Constant(0.1), 
    nonlinearity=nn.nonlinearities.LeakyRectify(leakiness))
...
def conv1d_layer(l, num_filters=16, filter_length=3):
    # _,_,h,w = l.get_output_shape()

    # reshape_bchw_to_bcs(l)

    l = conv1d_layer_base(l, num_filters=num_filters, filter_length=filter_length)

    # l = reshape_bcs_to_bchw(l, h, w)

    return l

def reshape_bcs_to_bchw(l, h, w):
    # s = seq_length
    b = batch_size
    _,c,s = l.get_output_shape()
    l = nn.layers.ReshapeLayer(l, (b,h,w,c,s))
    l = nn.layers.DimshuffleLayer(l, (0,4,3,1,2))
    l = nn.layers.ReshapeLayer(l, (b*s,c,h,w))
    return l

def reshape_bchw_to_bcs(l, s):
    # s = seq_length
    b = batch_size
    _,c,h,w = l.get_output_shape()
    l = nn.layers.ReshapeLayer(l, (b,s,c,h,w) )
...
def reshape_bschw_to_bcs(l):
    # s = seq_length
    _,s,c,_,_ = l.get_output_shape()
    l = nn.layers.DimshuffleLayer(l, (0,3,4,2,1))
    l = nn.layers.ReshapeLayer(l, (-1, c, s))
    return l

def reshape_bschw_to_bchw(l):
    b,s,c,h,w = l.get_output_shape()
    l = nn.layers.ReshapeLayer(l, (b*s,c,h,w))
    return l
